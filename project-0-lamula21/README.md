...
Due: February 4, 2023 at 11:59 PM
...
First, you will need to clone this repository to your local filesystem. 
To do this, you will first need to install git and then make a github token. 

To install git you can do the following:
 + If you are using WSL, use your subsystem's package manager. For example `apt install git`
 + If you are using linux, use your system's package manager. For example `apt install git`
 + If you are using MacOS, you can install XCode. Alternatively if you run `git --version` in your terminal, it will prompt you to install git if it is not already installed. 

Once installed, you need to make an access token to access private repositories.
 See [https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/about-authentication-to-github](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/about-authentication-to-github) how to do this. 
I would reccomend authenticating with command line using SSH.

Once that is all setup you can then run:

```
git clone https://github.com/cmsc330-umd/project-0-<github-username>
```
...
1. Firstly, install the basic dependencies:
    ...
    - Run `sudo apt install ruby-dev ocaml ocaml-native-compilers camlp4 make m4 curl graphviz libssl-dev pkg-config`
2. Install some Ruby gems for testing
    - Run `sudo gem install --no-document minitest`
3. Install and initialize the OCaml package manager
    - First see if your system has the opam package already
        - for example: `apt get-install opam`
        - check the version: `opam --version`. Check to make sure the version is 2.X
        - if both of these steps work with the right version, move to step 4. Otherwise uninstall and follow the next step to get the latest version
...
4. Initialize OCaml
    ...
    - If you are already on 4.13.X, you can skip to #5
    - Run `opam update`
...
4. Install and initialize the OCaml package manager
    - First see if your system has the opam package already
        - for example: `apt get-install opam`
        - check the version: `opam --version`. Check to make sure the version is 2.X
        - if both of these steps work with the right version, move to step 5. Otherwise uninstall and follow the next step to get the latest version
    - Run `sh <(curl -sL https://raw.githubusercontent.com/ocaml/opam/master/shell/install.sh)` (when prompted for the installation location, just hit enter to select the default)
        - Run `opam --version`.  You should be on version 2 (followed by some versions, just make sure the major version is 2).  Check out [the manual](https://opam.ocaml.org/doc/Install.html) if this is not the case, you may have to follow special directions for your particular operating system and version.
        - If you encounter any issues, or are running a different flavor of linux, check out [the manual](https://opam.ocaml.org/doc/Install.html)
