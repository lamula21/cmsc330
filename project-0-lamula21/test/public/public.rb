...
VERSION = /(\d+(\.\d+))/
OCAML_VERSION = ["4.13","4.14"]
OPAM_VERSION = ["2.1","2.0","2.2"]
...
class PublicTests < Minitest::Test
  def test_public_ocaml
    ...
    assert(OCAML_VERSION.include?(ocaml_version), wrong_version("OCaml"))
  end

  def test_public_opam
    assert(opam_version, not_installed("OPAM"))
    assert(OPAM_VERSION.include?(opam_version), wrong_version("OPAM"))
  end
