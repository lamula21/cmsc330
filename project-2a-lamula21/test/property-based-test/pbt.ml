...
let test_rev_tup =
  Test.make ~name:"test_rev_tup" ~count:25 (triple int int int)
    (fun (x, y, z) -> rev_tup (x, y, z) = (z, y, x))

(* Reversing twice gives you the same *)
let test_rev_tup_twice =
  Test.make ~name:"test_rev_twice" ~count:25 (triple int int int)
    (fun (x, y, z) -> rev_tup (rev_tup (x, y, z)) = (x, y, z))

(* All numbers of the form 2x+1 are odd *)
let test_is_even_odds =
  Test.make ~name:"test_is_even_odds" ~count:10 (int_range (-1000000) 1000000)
    (fun x -> not (is_even ((2 * x) + 1)))

(* All numbers of the form 2x are even *)
let test_is_even_evens =
  Test.make ~name:"test_is_even_evens" ~count:10 (int_range (-1000000) 1000000)
    (fun x -> is_even (2 * x))

(* Product of two numbers is only even if one
   operand is even *)
let test_is_even_multiplicativity =
  Test.make ~name:"test_is_even_multiplicativity" ~count:10
    (pair small_int small_int) (fun (x, y) ->
      if is_even x || is_even y then is_even (x * y) else not (is_even (x * y)))

(* Sum of two numbers is only odd if ones is even
   and the other is odd *)
let test_is_odd_additivity =
  Test.make ~name:"test_is_odd_additivity" ~count:10 (pair small_int small_int)
    (fun (x, y) ->
      if is_even x <> is_even y then not (is_even (x + y)) else is_even (x + y))

(* Area *)
let test_area_positive =
  Test.make ~name:"test_area_positive" ~count:10
    (quad small_int small_int small_int small_int) (fun (a, b, c, d) ->
      area (a, b) (c, d) >= 0)

(* Make sure that fib(x+1) >= fib(x) *)
let test_fib_inc =
  Test.make ~name:"test_fib_inc" ~count:10 (int_range 0 25) (fun x ->
      fibonacci (x + 1) >= fibonacci x)

(* Make sure that fib(x) is always positive *)
let test_fib_geq_z =
  Test.make ~name:"test_fib_geq_z" ~count:20 (int_range 0 25) (fun x ->
      fibonacci x >= 0)

(* Powers should always be positive *)
let test_pow_pos =
  Test.make ~name:"test_pow_pos" ~count:30 (pair small_int small_int)
    (fun (a, b) ->
      let x = a mod 100 in
      let y = b mod 10 in
      pow x y > 0)
...
(* (a^b)^b = a^(b^2) *)
let test_pow_twice =
  Test.make ~name:"test_pow_twice" ~count:30 (pair small_int small_int)
    (fun (a, b) ->
      let x = a mod 100 in
      let y = b mod 10 in
...
(* Reversing twice gives you the same *)
let test_rev_lst =
  Test.make ~name:"test_rev_lst" ~count:30 (list int) (fun lst ->
